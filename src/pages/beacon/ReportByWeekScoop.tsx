...
const ReportByWeekScoop = () => {
  ...
  const planWeek = useMemo(() => {
    ...
    const planDataBlending = planDayData.filter(
      (day) => day.type === "blending",
    );
    const planDataModificado = planDayData.filter(
      (day) => day.type === "modificado",
    );
    const allWeekDates: string[] = getWeekDates(dateFilter[0].startDate);

    const blendingGroupedByDate = planDataBlending.reduce(
      (acc, day) => {
    ...
    const modificadoGroupedByDate = planDataModificado.reduce(
      (acc, day) => {
        const dateKey = planDayDateParser(day.date);
        if (!acc[dateKey]) {
          acc[dateKey] = { date: dateKey, tonnage: 0 };
  ...
  const baseStats = useMemo(() => {
    if (!filteredData || !mineralData) {
      return {
        totalUnits: filteredData.filter((unit) => unit.trips.length > 0).length,
    ...
    const avgDurationSuperficieTrips =
      allTrips
        .filter((trip) => trip.location === "Superficie")
    ...
    const avgDurationSubterraneoTrips =
      allTrips
        .filter((trip) => trip.location === "Subterraneo")
        .reduce((avg, trip) => avg + trip.tripDurationMin, 0) /
      allTrips.filter(
    ...
    const totalMantanceTimeMin = filteredData.reduce(
  ...
  const tripsByDate = useMemo(() => {
    ...
    const allTrips = filteredData.flatMap((unitGroup) => unitGroup.trips);

    const grouped = allTrips.reduce(
      (acc: Record<string, BeaconUnitTrip[]>, trip) => {
    ...
    const result = allDatesInRange.map((date) => {
      ...
      const capitalizedDay =
        dayAbbrev.charAt(0).toUpperCase() + dayAbbrev.slice(1);
  ...
  return (
    <div className="grid grid-cols-[1fr_5fr] flex-1 w-full gap-4">
      <PageHeader
        title="Carguío por Turno Esteril"
...
export default ReportByWeekScoop;
