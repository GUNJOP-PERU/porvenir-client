...
export default function Tracking() {
  ...
  const trackingByArea = tracking.reduce((acc, item) => {
    if (!acc[item.area]) acc[item.area] = [];
    acc[item.area].push(item);
    return acc;
  }, {});

  console.log(trackingByArea);
  ...
  return (
    <div className="h-screen w-full bg-contain bg-no-repeat bg-center bg-[url('/map.png')] flex flex-wrap">
     <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex gap-3">
     {Object.keys(trackingByArea).map((area) => (
        <div key={area} className="flex flex-col gap-1">
          <div className="flex items-center gap-1">
            <div className="text-sm font-bold bg-[#417505] px-2 py-1 rounded-md text-white">
              {area}
            </div>
            <div className="text-sm font-bold bg-[#4a90e2] px-2 py-1 rounded-md text-white">
              {trackingByArea[area].length}
            </div>
          </div>
          <div className="flex flex-col w-fit gap-1">
            {trackingByArea[area].map((item, index) => (
              <div
                key={index}
                className="bg-black/70 flex flex-col gap-1 p-1.5 rounded-lg"
              >
                <div className="text-xs bg-green-500 text-white py-1.5 px-2 rounded-md">
                  STN-CV-{item.name}
                  {/* <TruckIcon className="h-10" /> */}
                </div>
      ...
      {/* <div>
        <div className="flex items-center gap-2">
          ...
          <div className="text-sm font-bold bg-[#4a90e2] px-2 py-1 rounded-md text-white">
            cantidad: {faja4.length}
          </div>
        </div>
        <div>
          {faja4.map((item, index) => (
            <div
              key={index}
              className="bg-black/80 flex flex-col gap-1 p-2 rounded-md"
            >
              <div className="text-xs bg-green-500 text-white py-1.5 px-2 rounded-md flex items-center gap-2">
                STN-CV-{item.name}
                <TruckIcon className="h-10" />
              </div>
              <span className="text-[10px] leading-none text-zinc-300">
      ...
      <div>
        <div className="flex items-center gap-2">
          ...
          <div className="text-sm font-bold bg-[#4a90e2] px-2 py-1 rounded-md text-white">
            cantidad: {cancha.length}
          </div>
        </div>
        <div>
          {cancha.map((item, index) => (
            <div
              key={index}
              className="bg-black/80 flex flex-col gap-1 p-2 rounded-md"
            >
      ...
      <div>
        <div className="flex items-center gap-2">
          ...
          <div className="text-sm font-bold bg-[#4a90e2] px-2 py-1 rounded-md text-white">
            cantidad: {pahuaypite.length}
          </div>
        </div>
        <div>
          {pahuaypite.map((item, index) => (
            <div
              key={index}
              className="bg-black/80 flex flex-col gap-1 p-2 rounded-md"
            >
      ...
      <div>
        <div className="flex items-center gap-2">
          ...
          <div className="text-sm font-bold bg-[#4a90e2] px-2 py-1 rounded-md text-white">
            cantidad: {tallerSaturno.length}
          </div>
        </div>
        <div>
          {tallerSaturno.map((item, index) => (
            <div
              key={index}
              className="bg-black/80 flex flex-col gap-1 p-2 rounded-md"
            >
      ...
      <div>
        <div className="flex items-center gap-2">
          ...
          <div className="text-sm font-bold bg-[#4a90e2] px-2 py-1 rounded-md text-white">
            cantidad: {bocamina_1.length}
          </div>
        </div>
        <div>
          {bocamina_1.map((item, index) => (
            <div
              key={index}
              className="bg-black/80 flex flex-col gap-1 p-2 rounded-md"
            >
      ...
      <div>
        <div className="flex items-center gap-2">
          ...
          <div className="text-sm font-bold bg-[#4a90e2] px-2 py-1 rounded-md text-white">
            cantidad: {bocamina_2.length}
          </div>
        </div>
        <div>
          {parqueo.map((item, index) => (
            <div
              key={index}
              className="bg-black/80 flex flex-col gap-1 p-2 rounded-md"
            >
      ...
      <div>
        <div className="flex items-center gap-2">
          <div className="text-sm font-bold bg-[#417505] px-2 py-1 rounded-md text-white">
            area: PARQUEO VOLQUETES
          </div>
          <div className="text-sm font-bold bg-[#4a90e2] px-2 py-1 rounded-md text-white">
            cantidad: {parqueo.length}
          </div>
        </div>
        <div>
          {parqueo.map((item, index) => (
