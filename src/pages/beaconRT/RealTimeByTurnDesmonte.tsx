...
const RealTimeByHourRT = () => {
  ...
  const {
    ...
    "trip-group-by-current-day-truck-desmonte-rt",
  ...
  const planDay = useMemo(() => {
    ...
    const filteredPlanData = planData.filter(
      (day) =>
        day.shift === shiftFilter && planDayDateParser(day.date) === currentDate && day.phase === "desmonte"
    );
    return {
  ...
  const baseStats = useMemo(() => {
    ...
    const totalTMNight = nightTrips * baseData.desmonte;
              ...
              actions={
                <div className="flex flex-row gap-2">
                  <div className="flex flex-row items-center gap-1">
                    <span className="flex bg-[#c19f55] w-2 h-2 rounded-full"/>
