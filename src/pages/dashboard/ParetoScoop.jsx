...
function ParetoScoop() {
    ...
    const fetchParetoScoop = useProductionStore((state) => state.fetchParetoScoop);
    const { scoopParetoProgress, scoopParetoProductive ,scoopParetoNoProductive} =
      useProductionStore();
  
    useEffect(() => {
      if (scoopParetoProgress.length === 0) {
        fetchParetoScoop();
      }
    }, [fetchParetoScoop, scoopParetoProgress]);
  
    useProductionWebSocket();
  
    console.log(scoopParetoProgress,"scoopParetoProgress");
    return (
      <>
        <div className="w-full flex flex-wrap justify-between px-4 py-2 bg-zinc-100/50 border border-zinc-100 rounded-xl gap-2">
          ...
          />
          <CardItem
            value={scoopParetoProgress?.data?.avg_planned_activities?.toFixed(1) || 0 }
            title="Prom. Actividad Programada"
            valueColor="text-[#B16940]"
            unid={"h"}
          />
          <CardItem
            value={scoopParetoProgress?.data?.avg_unplanned_activities?.toFixed(1) || 0 }
            title="Prom. Actividad No Programada"
            valueColor="text-[#B16940]"
            unid={"h"}
          />
          <CardItem
            value={scoopParetoProgress?.data?.avg_maintenance_activities?.toFixed(1) || 0}
          ...
          />
          <CardItem
            value={scoopParetoProgress?.data?.avg_service_activities?.toFixed(1) || 0}
